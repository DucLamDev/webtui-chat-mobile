...
M12 đã có Play-first foundation: Play Console doc, privacy policy draft, download page spec, source static page ở `portal/download/`, manifest example và `tool/check_mobile_release.dart` để CI bắt thiếu sót release. AAB đi qua Play Internal/Closed trước; APK do CI ký bằng upload key chỉ là artifact QA, không được public nếu khác Play app-signing certificate. Fallback tại `download.vpsttt.com/download/` chỉ bật với universal APK xuất từ Play hoặc artifact đã chứng minh cùng signer. Cần thao tác ngoài repo để tạo Play Console app, enroll Play App Signing, public privacy/support URL, upload AAB, chạy Pre-launch report và triển khai path tải thật.
