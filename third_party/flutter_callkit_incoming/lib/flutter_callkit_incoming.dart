...
void _flutterCallkitIncomingCallbackDispatcher() {
  ...
  unawaited(backgroundChannel.invokeMethod<void>('initialized'));
}
...
class FlutterCallkitIncoming {
  ...
  /// Returns Android call actions that happened before Flutter attached.
  ///
  /// Entries remain durable until [ackPendingCallAction] is called after the
  /// authenticated app has successfully applied the matching backend action.
  static Future<List<Map<String, dynamic>>> getPendingCallActions() async {
    final result =
        await _channel.invokeMethod<Object?>('getPendingCallActions');
    if (result is! List) {
      return const [];
    }
    return result
  ...
  /// Removes one process-death-safe Android action after successful handling.
  static Future<void> ackPendingCallAction(String actionId) async {
    await _channel.invokeMethod<void>('ackPendingCallAction', {
      'action_id': actionId,
    });
  }
  ...
  /// On Android, starts a guarded ongoing-call foreground service.
  static Future<void> startCall(CallKitParams params) async {
    await _channel.invokeMethod('startCall', params.toJson());
  }

  /// Promote an accepted Android call from its initial phoneCall-only
  /// foreground service to microphone/camera after visible runtime grants.
  ///
  /// This is a WebTUI safety extension. It is intentionally invoked only on
  /// Android; iOS uses CallKit's native audio-session lifecycle.
  static Future<void> activateCallMedia(CallKitParams params) async {
    await _channel.invokeMethod('activateCallMedia', params.toJson());
  }
