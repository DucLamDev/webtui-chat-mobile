...
class FlutterCallkitIncomingPlugin : FlutterPlugin, MethodCallHandler, ActivityAware,
    ...
    companion object {
        ...
        private fun send(context: Context?, event: String, body: Map<String, Any?>) {
            ...
            } else if (context != null && event in setOf(
                    CallkitConstants.ACTION_CALL_DECLINE,
                    CallkitConstants.ACTION_CALL_ENDED,
                    CallkitConstants.ACTION_CALL_TIMEOUT,
                )
            ) {
                // The receiver holds a bounded goAsync lease for this prompt
                // attempt. WorkManager owns persistent retry after the lease.
                Log.d(TAG, "Sending leased background event: $event")
                CallkitBackgroundExecutor.send(context, event, body)
            }
        }
        ...
        try {
            when (call.method) {
                ...
                "getPendingCallActions" -> {
                    result.success(
                        CallkitPendingActionStore.pending(requireNotNull(context))
                    )
                }

                "ackPendingCallAction" -> {
                    val actionId = (call.arguments as? Map<*, *>)
                        ?.get("action_id")
                        ?.toString()
                        ?.trim()
                        .orEmpty()
                    if (actionId.isNotEmpty()) {
                        CallkitPendingActionStore.acknowledge(
                            requireNotNull(context),
                            actionId,
                        )
                        CallkitBackgroundExecutor.stopIfIdle(
                            requireNotNull(context),
                        )
                        CallkitActionCompletionRegistry.complete(actionId)
                        CallkitReceiverExecutionLease.complete(actionId)
                    }
                    result.success(true)
                }
                ...
                "startCall" -> {
                    ...
                    if (!CallkitNotificationService.canPromoteMedia(appContext, data.type > 0)) {
                        result.error(
                            "MEDIA_PERMISSION_REQUIRED",
                            "Start outgoing calls only while the app is visible and media permissions are granted.",
                            null
                        )
                        return
                    }
                    val bundle = data.toBundle().apply {
                        putBoolean(CallkitConstants.EXTRA_CALLKIT_MEDIA_ACTIVE, true)
                    }
                    context?.sendBroadcast(
                        CallkitIncomingBroadcastReceiver.getIntentStart(
                            appContext,
                ...
                "activateCallMedia" -> {
                    ...
                    if (!CallkitNotificationService.canPromoteMedia(appContext, data.type > 0)) {
                        result.error(
                            "MEDIA_PERMISSION_REQUIRED",
                            "Activate call media only while the app is visible and media permissions are granted.",
                            null
                        )
                        return
                    }
                ...
                "endCall" -> {
                    ...
                    if (currentCall != null && appContext != null) {
                    ...
                    // The active-call preference can lag a service start. Stop
                    // unconditionally so setup failures, races, and repeated
                    // terminal events cannot strand the ongoing-call FGS.
                    if (appContext != null) {
                        CallkitNotificationService.stopService(appContext)
                        callkitNotificationManager?.clearIncomingNotification(
                            currentCall?.toBundle() ?: data.toBundle(),
                            false
                        )
                    }
                    result.success(true)
                ...
                "endAllCalls" -> {
                    ...
                    removeAllCalls(context)
                    context?.let { CallkitNotificationService.stopService(it) }
                    result.success(true)
                }
