...
class CallkitConnection(
    ...
    override fun onAnswer() {
        ...
        emitTelecomAction(CallkitConstants.ACTION_CALL_ACCEPT)
        setActive()
    }
    ...
    override fun onReject() {
        ...
        emitTelecomAction(CallkitConstants.ACTION_CALL_DECLINE)
    ...
    override fun onDisconnect() {
        ...
        emitTelecomAction(CallkitConstants.ACTION_CALL_ENDED)
    ...
    /**
     * Route system Telecom/Bluetooth/watch actions through the same receiver
     * lifecycle as notification buttons. The marker prevents the receiver
     * from driving this Connection again and creating a feedback loop.
     */
    private fun emitTelecomAction(action: String) {
        try {
            val intent = CallkitIncomingBroadcastReceiver.getIntent(
                applicationContext,
                action,
                bundle,
            ).putExtra(
                CallkitIncomingBroadcastReceiver.EXTRA_ACTION_FROM_TELECOM,
                true,
            )
            applicationContext.sendBroadcast(intent)
        } catch (error: Exception) {
            // Persist directly as a final fallback if OEM broadcast delivery
            // itself fails. Replay will occur after Flutter/auth are ready.
            CallkitPendingActionStore.record(applicationContext, action, bundle)
            Log.w(TAG, "Telecom action delivery failed: ${error.message}")
        }
    }
